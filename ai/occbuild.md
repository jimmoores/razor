# Razor Toolchain Drivers Analysis

This document provides an analysis of the primary command-line tools used to build and manage occam-pi programs within the Razor ecosystem: `razor` and `occbuild`.

## 1. `razor` - The Main Compiler Driver

`razor` is a shell script acting as the primary front-end driver for the compilation system, functioning analogously to `gcc` for C. It orchestrates the pipeline of compiling, translating, assembling, and linking occam-pi code.
...
4.  **Linking (`ld` / `gcc`)**: Links the object files with the Razor runtime system (`CCSP`) and other libraries to produce an executable or a shared library.
...
### Configuration
The script is generated from `razor.in` by `configure`, injecting paths to the installed compiler binaries (`occ21`, `tranx86`), default library paths, and version information.

## 2. `occbuild` - The Build System Abstraction

`occbuild` is a Python script that provides a higher-level, standardized interface for building occam-pi components. It abstracts the differences between various underlying toolchains (native Razor, TVM, Tock) and provides a command-set tailored for building modules and libraries.
...
1.  **`razor`**: Wraps the `razor` shell script described above. Used for native compilation.
...
Unlike `razor`, which is a straight compiler driver, `occbuild` has logic to help manage library metadata:
...
*   It handles the `in-tree` build scenario, allowing the Razor build system to bootstrap itself.
...
*   **Purpose**: Provides a quick way to bootstrap a build for a simple project without writing manual Makefiles, though for complex projects (like Razor itself), Automake or `occbuild` is preferred.

## Relationship Summary

*   **User Level**: Users typically run `razor` directly to compile simple programs.
*   **Build System Level**: `occbuild` is heavily used by the Razor build system (Automake) to build the standard libraries and modules in a platform-agnostic way.
*   **Execution**: `occbuild --toolchain=razor` calls `razor`, which calls `occ21` -> `tranx86` -> `gcc`.
