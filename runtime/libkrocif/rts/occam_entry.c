...
extern void _occam_start (void) __attribute__((weak));	/* occam entry point */

/* These symbols are generated by tranx86 for compiled occam programs.
 * For pure CIF C programs (like cift15) that don't compile occam code,
 * they won't be defined.  Use weak linkage so they default to 0. */
extern word _wsbytes __attribute__((weak));	/* occam workspace size */
extern word _vsbytes __attribute__((weak));	/* occam vectorspace size */
extern word _msbytes __attribute__((weak));	/* occam mobilespace size */
extern int _occam_tlp_iface __attribute__((weak));	/* translator added description of the top-level process */
extern int _occam_errormode __attribute__((weak));	/* non-zero for STOP errormode */
