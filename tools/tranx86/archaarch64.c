...
/* CCSP_DIRECT_CALL: when defined, kernel calls pass ALL parameters as
 * standard C function arguments (p0, p1, ..., sched, Wptr) instead of
 * using the legacy cparam[] memory block for extra arguments.
 * This must match the same flag in the runtime's sched_asm_inserts.h. */
 ...
 *
 *\tThe CIF C function receives Wptr+1 word in x0 (matching i386 convention).
 *\tWe pass Wptr+1 via LEA into x0 rather than adjusting REG_WPTR, because
 *\tthe RTL optimizer would eliminate a temporary REG_WPTR adjustment.
 *\tThe FLABEL/marker/sched saves use offsets relative to Wptr (not Wptr+1).
 *
 *\tPost-call: Wptr is NOT adjusted (pst_first is a no-op from Wptr's
 *\tperspective). pst_last does net +4 words (matching ccall/bcall).
 *\tThe surrounding occam I_CALL/I_RET code does -4/+4 which cancels.
 *\tThe I_CALL AJW and compose_cif_call's pst_last together give the
 *\tstandard -4+4=0 net on Wptr, then the occam code's own +N gives
 *\tthe final workspace adjustment.
 *
 *\tThe CIF asm stub saves C stack at wptr[-10] (byte -80 from the
 *\tWptr+1 value passed to the C function), i.e. original_Wptr[-9].
 */
static void compose_cif_call_aarch64 (tstate *ts, int inlined, char *name, ins_chain **pst_first, ins_chain **pst_last)
{
	char sbuf[256];
	 ...
	 * The C function receives wptr = REG_WPTR + 1 word. It reads sched
	 * via wptr[SchedPtr] = wptr[-7] = REG_WPTR[-6]. So we save sched
	 * at REG_WPTR[-6], not [-7]. Same shift for FLABEL and marker:
	 * FLABEL at REG_WPTR[-8] (C reads wptr[-9] = REG_WPTR[-8])
	 * marker at REG_WPTR[-7] (C reads wptr[-8] = REG_WPTR[-7])
	 * sched  at REG_WPTR[-6] (C reads wptr[-7] = REG_WPTR[-6]) */
...
static void compose_aarch64_kcall (tstate *ts, const int call, const int regs_in, const int regs_out)
{
	...
	/* CCSP calling convention for kernel functions.
	 * Two conventions are supported:
	 *
	 * CCSP_DIRECT_CALL (new): all parameters as normal C arguments:
	 *   kernel_func(p0, p1, ..., sched, Wptr)
	 *   where p0..pN go in x0..xN-1, sched in xN, Wptr in xN+1.
	 *
	 * Legacy (old): first param + cparam[] memory block:
	 *   kernel_func(param0, sched, Wptr)
	 *   where param0 is x0, sched is x1, Wptr is x2,
	 *   and additional params are in sched->cparam[0..N].
	 */
	 ...
	 */
	{
		 ...
		 * Process in reverse order to avoid clobbering: if cregs[1] happens
		 * to be in x0, we must not overwrite x0 with cregs[0] first. */
		for (i = regs_in - 1; i >= 0; i--) {
			if (i < 3 && cregs[i] != REG_UNDEFINED) {
				switch (constmap_typeof (cregs[i])) {
				case VALUE_CONST:
					add_to_ins_chain (compose_ins (INS_MOVE, 1, 1, ARG_CONST, constmap_regconst (cregs[i]), ARG_REG, i));
					break;
				default:
					add_to_ins_chain (compose_ins (INS_MOVE, 1, 1, ARG_REG, cregs[i], ARG_REG, i));
					break;
				}
			} else if (i >= 3) {
				/* cregs only has 3 slots (a, b, c); inputs beyond index 2
				 * are not currently generated by the ETC compiler. */
	...
	/* Build the call instruction with all argument registers listed */
	{
		...
		switch (total_args) {
		case 2: /* 0 inputs: sched=x0, Wptr=x1 */
			call_ins = compose_ins (INS_CALL, 3, 0, ARG_NAMEDLABEL, entrypoint_name, ARG_REG, REG_X0, ARG_REG, REG_X1);
			break;
		case 3: /* 1 input: p0=x0, sched=x1, Wptr=x2 */
		...
		default:
			...
			call_ins = compose_ins (INS_CALL, 7, 0, ARG_NAMEDLABEL, entrypoint_name, ARG_REG, REG_X0, ARG_REG, REG_X1, ARG_REG, REG_X2, ARG_REG, REG_X3, ARG_REG, REG_X4);
			break;
		}
	}
	/*}}}*/
#else /* !CCSP_DIRECT_CALL */
	/*{{{  legacy: first param in x0, extras via sched->cparam[], sched=x1, Wptr=x2 */
