...
fprintf (stderr, "MAGIC IOSPACE! (store-byte) %d --> [%d]\n", ts->stack->old_b_reg, ts->stack->old_a_reg);
	...
	case I_LW:
		 ...
		 * On 32-bit targets (BytesPerWord == 4), INT == word, so use INS_MOVE. */
#if (BytesPerWord > 4)
		add_to_ins_chain (compose_ins (INS_MOVE32, 1, 1, ARG_REGIND, ts->stack->old_a_reg, ARG_REG, ts->stack->a_reg));
#else
		add_to_ins_chain (compose_ins (INS_MOVE, 1, 1, ARG_REGIND, ts->stack->old_a_reg, ARG_REG, ts->stack->a_reg));
#endif
		constmap_remove (ts->stack->a_reg);
		ts->stack->must_set_cmp_flags = 1;
		break;
	...
	case I_SW:
		 ...
		 * On 32-bit targets, INT == word, so use INS_MOVE. */
#if (BytesPerWord > 4)
		add_to_ins_chain (compose_ins (INS_MOVE32, 1, 1, ARG_REG, ts->stack->old_b_reg, ARG_REGIND, ts->stack->old_a_reg));
#else
		add_to_ins_chain (compose_ins (INS_MOVE, 1, 1, ARG_REG, ts->stack->old_b_reg, ARG_REGIND, ts->stack->old_a_reg));
#endif
		ts->stack->must_set_cmp_flags = 1;
		break;
		/*}}}*/
	...
	case I_XSWORD:
		 ...
		 * On 32-bit targets, this is a no-op (INT == word).
	...
	case I_XOR:
		...
		/*{{{  I_ADD -- addition*/
	case I_ADD:
		/* INT in occam is 32-bit; on 64-bit targets a full-width
		 * adds/subs reports V relative to the 64-bit signed range and
		 * misses 32-bit overflow.  Use INS_ADD32 / INS_SUB32 on
		 * aarch64 (32-bit adds/subs wN form) so the V flag reflects
		 * 32-bit overflow.  x64 already does INT arithmetic via
		 * INS_ADD/INS_SUB which the x64 emitter renders as 32-bit
		 * addl/subl when needed; keeping its existing path avoids the
		 * 3-vs-2 operand convention mismatch on its INS_ADD32 case. */
