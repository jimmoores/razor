...
static int disassemble_code (ins_chain *ins, FILE *outstream, int regtrace)
{
	...
	if (sizeof (codes) / sizeof (codes[0]) != sizeof (code_sizes) / sizeof (code_sizes[0])) {
		fprintf (stderr, "%s: fatal: assertation (sizeof(codes) == sizeof(code_sizes)) (%d != %d) failed\n", progname, (int) (sizeof (codes) / sizeof (codes[0])), (int) (sizeof (code_sizes) / sizeof (code_sizes[0])));
		return -1;
	}
